...
$f(q)$ being close to a gaussian with compact support: $f(q) \gt 0$ for $0 \le q \lt 1$ or $0 \le q \lt 2$ (__Eq 5__) <br>
with
$q = \frac{ || \bold{x}_i - \bold{x}_j || }{h}$
...
$\nabla^2\bold{v}_i$ can be obtained using __Eq 8__ (p.2)

___

With $\bold{a}_i = \frac{\mathbf{F}_i}{m_i}$, time integration can be used to deduce velocity $\mathbf{v}_i$ and position $\mathbf{x}_i$
